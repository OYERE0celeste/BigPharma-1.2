...
router.get("/export", async (req, res, next) => {
  try {
    ...
    let csv = "Date,Produit,SKU,Lot,Type,Quantite,Avant,Apres,Raison,Utilisateur\n";
    
    for (const m of mouvements) {
      csv += `"${m.createdAt.toISOString()}","${m.produitId?.name || 'N/A'}","${m.produitId?.sku || ''}","${m.lotNumber}","${m.type}",${m.quantite},${m.beforeQuantity},${m.afterQuantity},"${m.reason}","${m.utilisateur}"\n`;
    }
